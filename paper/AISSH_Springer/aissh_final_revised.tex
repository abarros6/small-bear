...
\setlist{nosep}
\setlength{\intextsep}{2pt}% tighten float-to-text gaps throughout (default class spacing left large gaps around tables/figures)
...
This work introduces an age-adaptive pediatric conversational system composed of two principal artifacts: the \texttt{small-bear} conversational model, an age-stratified response generator targeting children (ages 5--11) and adolescents (ages 12--18) in a virtual reality (VR) hospital setting, and the \texttt{guard-bear} safety layer, an upstream input classifier. Both components run locally on consumer-grade hardware (Apple Mac Mini M4, 16~GB RAM) to eliminate external cloud dependencies and data transmission risks.

Deploying open-ended text interaction for pediatric populations requires specialized input screening. Standard prompt-injection classifiers, such as \texttt{Prompt-Guard-86M}~\cite{promptguard2024}, focus primarily on system-prompt overrides and developer threat models. Consequently, the unadapted \texttt{Prompt-Guard-86M} classifier fails to handle pediatric domain inputs, which consist of short first-person queries, benign out-of-scope questions, and developmental language mixed with out-of-domain content. Fine-tuning the \texttt{guard-bear} classifier resolves this transfer failure, establishing robust domain-specific safety gating.

To construct the \texttt{small-bear} conversational assistant, Llama 3.2 Instruct models~\cite{dubey2024llama3} (1B and 3B parameter scales) were adapted using Low-Rank Adaptation (LoRA)~\cite{hu2022lora} and QLoRA~\cite{dettmers2023qlora}. Following the LIMA hypothesis~\cite{zhou2023lima}, the model was trained on 1,123 synthetic instruction-response pairs derived from institutional patient resources. We systematically evaluate two adapter parameterizations---Standard ($r=8$, 16 layers) and Fast ($r=4$, 8 layers)---across model scales and age roles.
...
\begin{itemize}
    \item \textbf{Safety Transfer Analysis and Gating}: We demonstrate that general-purpose safety classifiers fail to transfer to pediatric conversational domains (ROC-AUC 0.448 for unmodified \texttt{Prompt-Guard-86M}) and show that fine-tuning the \texttt{guard-bear} classifier restores high discriminative performance ($0.9992\pm0.0005$ ROC-AUC across 5 seeds).
...
\subsection{\texttt{small-bear} Model Performance: Readability, Latency, and Style Separation}\label{sec:responsemodel}
...
\subsection{\texttt{guard-bear} Safety Classification Performance}\label{sec:guardresults}

Evaluating \texttt{Prompt-Guard-86M} (threshold 0.50) on the 608-example held-out test set demonstrates that general-purpose input safety classifiers fail to transfer to pediatric interaction domains (Table~\ref{tab:guardheadline}). \texttt{Prompt-Guard-86M} performs below random chance (ROC-AUC 0.448). This performance occurs because \texttt{Prompt-Guard-86M}'s imperative-override representations do not align with pediatric input characteristics, leading to systematic over-classification of safe inputs as unsafe (flagging 292 of 299 benign samples). The confusion matrix (Table~\ref{tab:confusion}) makes this saturation failure explicit in a way the threshold-independent ROC-AUC score alone does not: \texttt{Prompt-Guard-86M}'s errors sit almost entirely in one cell (292 false positives), revealing an ``always unsafe'' failure mode rather than a genuine, if imperfect, discrimination signal.

Fine-tuning the \texttt{guard-bear} classifier resolves this transfer failure. Across a 5-seed evaluation campaign (Table~\ref{tab:guardheadline}), the fine-tuned model achieves an ROC-AUC of $0.9992\pm0.0005$ and an unsafe recall of $0.9883\pm0.0043$, meeting operational performance targets (recall $\geq 0.97$, precision $\geq 0.85$, F1 $\geq 0.90$, accuracy $\geq 0.92$) across all individual seeds. A TF-IDF with logistic regression baseline trained on identical data achieves 0.9911 ROC-AUC but yields higher total errors (32 vs.\ 6 errors at seed 42; average $\sim$7.8 errors/run across 5 seeds). Fine-tuning demonstrates substantial gains on challenging categories such as \texttt{gibberish\_malformed} inputs (recall increasing from 38\% to 92\%). The fine-tuned matrix (Table~\ref{tab:confusion}) is close to diagonal at both the single-seed and 5-seed-aggregate level, with errors split roughly evenly between the two off-diagonal cells rather than concentrated in one, as they are for \texttt{Prompt-Guard-86M}.
...
\begin{table}[!h]
  \centering
  \caption{Performance of the \texttt{guard-bear} classifier compared to \texttt{Prompt-Guard-86M} and the TF-IDF+LR baseline on the held-out test set (608 examples: 299 safe, 309 unsafe; deduplicated). The TF-IDF+LR baseline is trained on the same 2{,}836 training subset. The rightmost column reports mean $\pm$ SD across a 5-seed campaign (42, 123, 456, 789, 2024).}
  \label{tab:guardheadline}
  \small
  {\setlength{\tabcolsep}{4pt}%
  \begin{tabular}{@{}lcccc@{}}
    \toprule
    Metric & \texttt{Prompt-Guard} (t=0.50) & TF-IDF+LR & Seed 42 & \texttt{guard-bear} (5-seed) \\
    \midrule
...
\begin{table}[!h]
  \centering
  \caption{Confusion matrices (actual vs.\ predicted class) on the 608-example held-out test set (299 safe, 309 unsafe): unmodified \texttt{Prompt-Guard-86M} vs.\ the fine-tuned \texttt{guard-bear} classifier (seed 42, and summed across the 5-seed campaign). \texttt{Prompt-Guard-86M}'s errors sit almost entirely in one cell (292 false positives) -- an ``always unsafe'' saturation failure rather than genuine discrimination; \texttt{guard-bear} is close to diagonal at both the single-seed and 5-seed-aggregate level, with errors split roughly evenly between the two off-diagonal cells. Corresponding TF-IDF+LR baseline counts are TN=278, FP=21, FN=11, TP=298 (Table~\ref{tab:guardheadline} reports its aggregate metrics).}
  \label{tab:confusion}
  \small
  \begin{tabular}{@{}ll cc@{}}
    \toprule
    Classifier & Actual & Pred.\ Safe & Pred.\ Unsafe \\
    \midrule
    \multirow{2}{*}{\texttt{Prompt-Guard} (t=0.50)}                  & Safe   & 7    & 292  \\
                                                                      & Unsafe & 19   & 290  \\
    \midrule
    \multirow{2}{*}{\texttt{guard-bear} (seed 42)}                   & Safe   & 296  & 3    \\
                                                                      & Unsafe & 3    & 306  \\
    \midrule
    \multirow{2}{*}{\texttt{guard-bear} (5-seed sum, $n$=3040)}      & Safe   & 1474 & 21   \\
                                                                      & Unsafe & 18   & 1527 \\
    \bottomrule
  \end{tabular}
\end{table}

\label{sec:deployment}The combined system footprint operates efficiently on a single local machine. The \texttt{guard-bear} safety layer adds minimal processing latency (42.05ms average), enabling the complete pipeline---when paired with the Fast \texttt{small-bear} conversational assistant (1B)---to maintain sub-second response times.
...
The system pipeline processes input through two sequential stages (Figure~\ref{fig:architecture}). Raw text is first evaluated by the \texttt{guard-bear} safety layer. Inputs classified as unsafe are intercepted and returned with standard safety handling. Safe inputs pass through an age router provided by the host VR application, which selects the corresponding fine-tuned adapter on a frozen 4-bit quantized base model. System prompts are omitted during adapter inference to ensure that age-specific register is encoded directly within adapter weights.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{images/architecture_v2.pdf}
  \caption{System architecture diagram. The \texttt{guard-bear} safety layer (86M parameters) screens incoming text before routing safe queries to age-stratified LoRA adapters deployed on a frozen 4-bit Llama 3.2 base model.}
  \label{fig:architecture}
\end{figure}

The \texttt{guard-bear} classifier performs full parameter fine-tuning of an 86M parameter DeBERTa architecture to categorize inputs as safe (benign pediatric questions and out-of-scope curiosity) or unsafe (adversarial injections, adult themes, and malformed text). Single-query evaluation averages 42.05ms on the target hardware platform.

\subsection{Datasets}\label{subsec:responsedataset}

Training data for the \texttt{small-bear} conversational assistant was constructed from institutional patient education materials~\cite{hospitalguide}. The dataset comprises 1,123 instruction-response pairs split across two age targets (562 for ages 5--11; 561 for ages 12--18), supplemented by 100 validation examples and 50 training-only edge-case entries.

The \texttt{guard-bear} classifier training set combines synthetic pediatric queries with established safety corpora, yielding 4{,}053 unique, deduplicated samples (2{,}055 unsafe, 1{,}998 safe) partitioned into disjoint train (2,836), validation (609), and test (608) splits.

\subsection{Base Models, LoRA Configurations, and Training Protocol}\label{sec:trainprotocol}

Fine-tuning of the \texttt{small-bear} conversational model was conducted using 4-bit quantized Llama 3.2 Instruct base models (1B and 3B) via \texttt{mlx\_lm}~\cite{apple2023mlx}. LoRA updates ($\Delta W = BA$) were applied to linear projections across target layers with scaling factor fixed at \texttt{scale=20.0}. The \textbf{Standard} configuration parameterizes $r=8$ across the upper 16 layers, while the \textbf{Fast} configuration parameterizes $r=4$ across the upper 8 layers. Optimization used Adam with cosine learning rate decay (peak $1\times10^{-5}$, 100-step warmup, 600 total steps, batch size 4).

The \texttt{guard-bear} safety layer was fine-tuned from \texttt{Prompt-Guard-86M}~\cite{promptguard2024}. The classification decision threshold was optimized on validation performance, establishing an operational threshold of \textbf{0.94}.

\subsection{Evaluation Methodology}

Readability metrics for the \texttt{small-bear} model were evaluated using \texttt{textstat}~\cite{bansal2023textstat}, establishing Flesch-Kincaid (FK~$\leq$~7.0) as the primary benchmark alongside SMOG, Gunning Fog, Coleman-Liau, and Type-Token Ratio (TTR). Inter-role lexical separation was evaluated via 5-fold cross-validated TF-IDF logistic regression classifiers. The \texttt{guard-bear} classifier was evaluated on unsafe recall, precision, F1 score, accuracy, ROC-AUC, and subcategory recall.

\section{Discussion}\label{sec:discussion}

The failure of un-adapted safety classifiers on pediatric inputs underscores that \textbf{safety mechanisms cannot be assumed to transfer across domain populations without explicit fine-tuning}. \texttt{Prompt-Guard-86M}'s low ROC-AUC (0.448) demonstrates that developer-focused threat representations misclassify benign pediatric interactions. Fine-tuning the \texttt{guard-bear} safety layer eliminates these false positives while maintaining high unsafe recall ($0.9883\pm0.0043$).

For response generation, the Fast \texttt{small-bear} conversational assistant (1B) achieves the operational requirements of interactive VR deployment, sustaining an average latency of 0.97 seconds and a 62\% sub-second turn rate. While length-constrained system prompts enable base models to achieve comparable readability, fine-tuning provides predictable low-latency output without manual prompt engineering.
...
\item \textbf{Data and Code Availability:} Code and data for the \texttt{small-bear} conversational model are available at \url{https://github.com/abarros6/small-bear}. Code and data for the \texttt{guard-bear} classifier are available at \url{https://github.com/abarros6/guard-bear}.
