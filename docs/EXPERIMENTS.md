...
| Model | Llama 3.2 1B-Instruct-4bit, Llama 3.2 3B-Instruct-4bit, Qwen2-0.5B-Instruct-4bit |
| Role | `age_5_11` only (deployment target — halves run count) |
| Seed | 42, 1337 |

Total: 4 ranks × 3 models × 2 seeds = **24 runs**. Wall-clock ~10–12 h sequentially on M4.

Qwen2-0.5B is included because RESULTS_COMPARISON.md shows a split crossover signal on it:
FK pass rate favours Standard (74% vs 68%) but classifier favours Fast (0.960 vs 0.940).
The rank sweep will show whether this split is a rank-4 artifact or stable across ranks.

**Infrastructure:** configs generated by `scripts/gen_sweep_configs.py`, run via
`bash scripts/train_sweep.sh <config>` or `bash scripts/run_sweep_1.sh` for all 24 in sequence.
